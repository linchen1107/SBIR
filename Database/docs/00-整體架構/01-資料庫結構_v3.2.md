# SBIR 裝備資料庫中英文對照表 (V3.2)

## 📊 資料庫資訊

- **資料庫名稱**: sbir_equipment_db_v3
- **版本**: V3.2
- **用途**: 海軍裝備管理系統 + Web 應用整合 + NSN 申編搜尋系統
- **編碼**: UTF8
- **建立日期**: 2025-11-25
- **最後更新**: 2025-12-09
- **架構特點**:
  - Item 自我關聯 BOM 結構
  - UUID 主鍵
  - 擴展表設計
  - **雙 Schema 架構**:
    - `web_app`: 裝備管理系統 + Web 應用（19 個表格，原 public schema）
    - `public`: 搜尋系統（15 個表格 + 5 個視圖）
  - 整合 Web 應用使用者與申編單管理系統
  - 統一時間戳命名（date_created/date_updated）
  - EMU3000 維修物料匯入邏輯
  - NSN 申編搜尋系統完整整合
- **總表數**: 34 個資料表（web_app: 19 + public: 15）+ 5 個視圖

---

## 📋 欄位對照說明圖例

- 🔑 = 主鍵 (Primary Key)
- 🔗 = 外鍵 (Foreign Key)
- ⭐ = 必填欄位 (Required)
- 📝 = 選填欄位 (Optional)
- 🔄 = 自動產生 (Auto Generated)

---

## 📑 目錄

- [資料表總覽](#資料表總覽)
- [V3.2 變更說明](#v32-變更說明)
- [V3.1 整合變更說明](#v31-整合變更說明)
- [web_app Schema - 裝備管理表（13個）](#web_app-schema---裝備管理表13個)
  - [Supplier - 廠商主檔](#1-supplier-廠商主檔)
  - [Item - 品項主檔](#2-item-品項主檔)
  - [Item_Equipment_Ext - 裝備擴展表](#3-item_equipment_ext-裝備擴展表)
  - [Item_Material_Ext - 料件擴展表](#4-item_material_ext-料件擴展表)
  - [BOM - BOM主表](#5-bom-bom主表)
  - [BOM_LINE - BOM明細行](#6-bom_line-bom明細行)
  - [MRC - 品項規格表](#7-mrc-品項規格表)
  - [item_number_xref - 零件號碼關聯檔](#8-item_number_xref-零件號碼關聯檔)
  - [TechnicalDocument - 技術文件檔](#9-technicaldocument-技術文件檔)
  - [Item_Document_xref - 品項文件關聯檔](#10-item_document_xref-品項文件關聯檔)
  - [SupplierCodeApplication - 廠商代號申請表](#11-suppliercodeapplication-廠商代號申請表)
  - [CIDApplication - CID申請單](#12-cidapplication-cid申請單)
  - [item_emu3000_maintenance_ext - EMU3000維修物料擴展表](#13-item_emu3000_maintenance_ext-emu3000維修物料擴展表)
- [web_app Schema - Web 應用表（6個）](#web_app-schema---web-應用表6個)
  - [User - 使用者管理](#14-user-使用者管理)
  - [Application - 申編單主表](#15-application-申編單主表)
  - [ApplicationAttachment - 附件管理](#16-applicationattachment-附件管理)
  - [UserSession - 工作階段管理](#17-usersession-工作階段管理)
  - [ApplicationLog - 應用程式日誌](#18-applicationlog-應用程式日誌)
  - [AuditLog - 稽核日誌](#19-auditlog-稽核日誌)
- [public Schema - NSN 搜尋系統（15個表 + 5個視圖）](#public-schema---nsn-搜尋系統15個表--5個視圖)
- [資料表關聯圖](#資料表關聯圖)
- [時間戳命名策略](#時間戳命名策略)
- [EMU3000 資料匯入規則](#emu3000-資料匯入規則)

---

## 資料表總覽

### web_app Schema（19 個表格）

| 編號 | 英文表名 | 中文名稱 | 主鍵類型 | 時間戳命名 | 用途 |
|------|---------|---------|---------|-----------|------|
| **裝備管理表（13個）** | | | | | |
| 1 | Supplier | 廠商主檔 | SERIAL | date_created | 供應商/製造商基本資料 |
| 2 | Item | 品項主檔 ⭐ | UUID | date_created | 統一品項資料（核心表） |
| 3 | Item_Equipment_Ext | 裝備擴展表 | UUID (FK) | date_created | 裝備類型專用欄位 |
| 4 | Item_Material_Ext | 料件擴展表 | UUID (FK) | date_created | 料件類型專用欄位 |
| 5 | BOM | BOM主表 | UUID | date_created | BOM版本控制 |
| 6 | BOM_LINE | BOM明細行 ⭐ | UUID | date_created | Item自我關聯（元件清單） |
| 7 | MRC | 品項規格表 | UUID | date_created | 品項規格資料 |
| 8 | item_number_xref | 零件號碼關聯檔 | SERIAL | date_created | 品項-零件號-廠商多對多關聯 |
| 9 | TechnicalDocument | 技術文件檔 | SERIAL | date_created | 技術文件/手冊主檔 |
| 10 | Item_Document_xref | 品項文件關聯檔 | 複合鍵 | date_created | 品項-技術文件多對多關聯 |
| 11 | SupplierCodeApplication | 廠商代號申請表 | UUID | date_created | 廠商代號申請 |
| 12 | CIDApplication | CID申請單 | UUID | date_created | CID申請 |
| 13 | item_emu3000_maintenance_ext | EMU3000維修物料擴展 | UUID | date_created | EMU3000維修物料專屬 |
| **Web 應用表（6個）** | | | | | |
| 14 | User | 使用者管理 | UUID | date_created | 系統使用者帳號管理 |
| 15 | Application | 申編單主表 | UUID | date_created | 申編單完整資料（取代ApplicationForm） |
| 16 | ApplicationAttachment | 附件管理 | UUID | date_created | 申編單附件（BYTEA儲存） |
| 17 | UserSession | 工作階段管理 | VARCHAR | date_created | 使用者登入Session |
| 18 | ApplicationLog | 應用程式日誌 | UUID | timestamp | 應用程式運行日誌 |
| 19 | AuditLog | 稽核日誌 | UUID | date_created | 操作審計追蹤 |

**小計**: 19 個資料表（13 裝備管理 + 6 Web 應用）

---

### public Schema（15 個表格 + 5 個視圖）

| 編號 | 英文表名 | 中文名稱 | 主鍵類型 | 時間戳命名 | 用途 |
|------|---------|---------|---------|-----------|------|
| **FSG/FSC 分類系統（3個）** | | | | | |
| 1 | fsg | FSG 聯邦補給群組 | VARCHAR(2) | date_created | 聯邦補給群組分類 |
| 2 | fsc | FSC 聯邦補給分類 | VARCHAR(4) | date_created | 聯邦補給分類代碼 |
| 3 | inc_fsc_xref | INC-FSC 對應表 | 複合鍵 | date_created | INC 與 FSC 對應關係 |
| **NATO H6 系統（2個）** | | | | | |
| 4 | nato_h6_item_name | NATO H6 物品名稱 | VARCHAR(6) | date_created | NATO H6 標準物品名稱 |
| 5 | nato_h6_inc_xref | H6-INC 對應表 | 複合鍵 | date_created | H6 與 INC 對應關係 |
| **INC 系統（2個）** | | | | | |
| 6 | inc | INC 物品名稱代碼 | VARCHAR(5) | date_created | 物品名稱代碼主檔 |
| 7 | colloquial_inc_xref | 俗語-INC 對應表 | 複合鍵 | date_created | 俗語與 INC 對應 |
| **FIIG 系統（2個）** | | | | | |
| 8 | fiig | FIIG 識別指南 | VARCHAR(4) | date_created | 聯邦物品識別指南 |
| 9 | fiig_inc_xref | FIIG-INC 對應表 | 複合鍵 | date_created | FIIG 與 INC 對應 |
| **MRC 系統（4個）** | | | | | |
| 10 | mrc_key_group | MRC 群組 | VARCHAR(3) | date_created | MRC 群組分類 |
| 11 | mrc | MRC 需求代碼 | VARCHAR(5) | date_created | 物料需求代碼主檔 |
| 12 | fiig_inc_mrc_xref | FIIG-INC-MRC 對應表 ⭐ | SERIAL | date_created | 三元對應核心表（170萬筆） |
| 13 | mrc_reply_table_xref | MRC-回應表對應 | 複合鍵 | date_created | MRC 與回應表對應 |
| **回應系統（1個）** | | | | | |
| 14 | reply_table | 回應表主檔 | SERIAL | date_created | 申編回應選項（16萬筆） |
| **其他（1個）** | | | | | |
| 15 | mode_code_edit | 模式碼編輯指南 | VARCHAR(2) | date_created | 模式碼編輯規則 |

**小計**: 15 個資料表

#### 查詢視圖（5 個）

| 編號 | 視圖名稱 | 說明 | 用途 |
|------|---------|------|------|
| 1 | v_h6_inc_mapping | H6→INC 完整對應 | NATO H6 轉 INC 查詢 |
| 2 | v_inc_fiig_mapping | INC→FIIG 完整對應 | INC 轉 FIIG 查詢 |
| 3 | v_fiig_mrc_requirements | FIIG→MRC 申編需求 | 申編 MRC 需求查詢 |
| 4 | v_mrc_reply_options | MRC 回應選項 | MRC 回應選項查詢 |
| 5 | v_application_flow | 完整申編流程 | 端到端申編流程視圖 |

---

**總計**: 34 個資料表（web_app: 19 + public: 15）+ 5 個視圖

---

## V3.2 變更說明

### 🔄 主要變更

#### 1. ✅ 新增 EMU3000 資料匯入規則

**理由**:
- 支援 EMU3000 維修物料清單的匯入
- 定義總成與次總成的識別邏輯
- 透過 BOM 結構建立關聯

**影響**:
- 新增文件章節 [EMU3000 資料匯入規則](#emu3000-資料匯入規則)
- 不影響現有資料表結構，僅定義資料對應邏輯

#### 2. ⚡ Schema 架構重構與 NSN 系統整合 (2025-12-08/09)

**理由**:
- 為了更好的資料庫組織與功能隔離
- 整合完整的 NSN 申編搜尋系統
- 支援多模組擴充架構

**第一階段：Schema 重構 (2025-12-08)**:
- 原 `public` schema 更名為 `web_app`（19 個裝備管理表）
- 建立新的 `public` schema 用於 NSN 搜尋系統
- 所有裝備管理與 Web 應用表格移至 `web_app` schema

**第二階段：NSN 系統整合 (2025-12-09)**:
- 在 `public` schema 建立 15 個 NSN 核心表格
- 建立 5 個查詢視圖（v_h6_inc_mapping 等）
- 匯入超過 50 萬筆 NSN 資料（306.8 MB）
- 完成 FSG/FSC、NATO H6、INC、FIIG、MRC 等系統整合

**影響**:
- 資料表總數：19 → 34 個
- 新增 public schema 專門用於 NSN 申編搜尋
- web_app schema 專注於裝備管理與 Web 應用
- 雙 schema 架構實現功能分離與安全隔離


---

## V3.1 整合變更說明

### 🔄 主要變更

#### 1. ✅ 整合 Web 應用系統（新增 6 個表）

**理由**:
- 提供完整的使用者管理系統
- 詳細的申編單管理（50+ 欄位 vs 原本 7 欄位）
- 附件管理（BYTEA 儲存）
- 完整的審計追蹤

**影響**:
- 新增 User 表（使用者管理）
- Application 表取代舊的 ApplicationForm
- 新增 ApplicationAttachment（附件管理）
- 新增 UserSession（工作階段管理）
- 新增 ApplicationLog（應用程式日誌）
- 新增 AuditLog（稽核日誌）

#### 2. ✅ 統一時間戳命名

**理由**:
- 所有表統一使用 date_created/date_updated 命名
- 保持資料庫命名一致性
- 簡化觸發器函數邏輯

**影響**:
- Web 應用表改用 date_created/date_updated
- 統一的觸發器函數
- 整個資料庫使用相同的時間戳命名規範

#### 3. ✅ Application 與 Item 表關聯

**理由**:
- 申編單需要關聯到具體的品項
- 支援申編單的完整生命週期管理

**影響**:
- Application 表新增 item_uuid 欄位
- 外鍵關聯到 Item 表

---

## web_app Schema - 裝備管理表（13個）

> **Schema**: `web_app`  
> **說明**: 裝備管理核心表，包含廠商、品項、BOM、規格等資料  
> **原 Schema**: `public`（2025-12-08 重構前）

### 1. Supplier（廠商主檔）

**用途**: 管理供應商/製造商基本資料
**來源**: 19M, 20M

| 英文欄位名 | 中文名稱 | 資料類型 | 標記 | 說明 |
|-----------|---------|---------|------|------|
| supplier_id | 廠商ID | SERIAL | 🔑🔄 | 自動編號 |
| supplier_code | 廠商來源代號 | VARCHAR(20) | UNIQUE | 廠商代碼 |
| cage_code | 廠家登記代號 | VARCHAR(20) | UNIQUE | CAGE CODE |
| supplier_name_en | 廠家製造商（英文） | VARCHAR(200) | ⭐ | 英文廠商名稱 |
| supplier_name_zh | 廠商中文名稱 | VARCHAR(100) | 📝 | 中文廠商名稱 |
| agent_name | 代理商名稱 | VARCHAR(255) | 📝 | 代理商名稱（製造商填 NULL，代理商填入名稱） |
| country_code | 國家代碼 | VARCHAR(10) | 📝 | 國別代碼 |
| date_created | 建立時間 | TIMESTAMP | 🔄 | 記錄建立時間 |
| date_updated | 更新時間 | TIMESTAMP | 🔄 | 記錄更新時間 |

**備註**:
- `agent_name`: 若此供應商為製造商，則此欄位為 NULL；若為代理商，則填入代理商名稱

---

### 2. Item（品項主檔）⭐

**用途**: 統一管理所有品項（成品/半成品/原物料）
**V3.0 變更**: 合併 Equipment，使用 UUID 主鍵

| 英文欄位名 | 中文名稱 | 資料類型 | 標記 | 說明 |
|-----------|---------|---------|------|------|
| item_uuid | 品項UUID | UUID | 🔑🔄 | 自動生成 UUID |
| item_code | 統一識別碼 | VARCHAR(50) | ⭐ UNIQUE | CID 或 NIIN |
| item_name_zh | 中文品名 | VARCHAR(100) | ⭐ | 品項中文名稱 |
| item_name_en | 英文品名 | VARCHAR(200) | ⭐ | 品項英文名稱 |
| item_type | 品項類型 | VARCHAR(10) | ⭐ | FG/SEMI/RM |
| uom | 基本計量單位 | VARCHAR(10) | 📝 | EA/SET/LOT等 |
| status | 狀態 | VARCHAR(20) | 📝 | Active/Inactive |
| date_created | 建立時間 | TIMESTAMP | 🔄 | 記錄建立時間 |
| date_updated | 更新時間 | TIMESTAMP | 🔄 | 記錄更新時間 |

**約束**:
- `item_type IN ('FG', 'SEMI', 'RM')`
- `status IN ('Active', 'Inactive')`

**索引**:
- `idx_item_code` - 識別碼索引
- `idx_item_type` - 類型索引
- `idx_item_status` - 狀態索引

---

### 3. Item_Equipment_Ext（裝備擴展表）

**用途**: 裝備類型專用欄位（FG 類型）
**來源**: 原 Equipment 表

| 英文欄位名 | 中文名稱 | 資料類型 | 標記 | 說明 |
|-----------|---------|---------|------|------|
| item_uuid | 品項UUID | UUID | 🔑🔗 | 外鍵連結至 Item |
| equipment_type | 裝備形式 | VARCHAR(50) | 📝 | 裝備型號/型式 |
| ship_type | 艦型 | VARCHAR(50) | 📝 | 適用艦型 |
| usage_location | 裝設地點 | VARCHAR(100) | 📝 | 安裝位置 |
| parent_equipment_zh | 上層適用裝備中文名稱 | VARCHAR(100) | 📝 | 父裝備中文名 |
| parent_equipment_en | 上層適用裝備英文名稱 | VARCHAR(200) | 📝 | 父裝備英文名 |
| parent_cid | 上層CID | VARCHAR(50) | 📝 | 父裝備識別碼 |
| eswbs_code | 族群結構碼HSC | VARCHAR(20) | 📝 | ESWBS（五碼） |
| system_function_name | 系統功能名稱 | VARCHAR(200) | 📝 | 系統功能說明 |
| installation_qty | 同一類型數量 | INT | 📝 | 單艦裝置數量 |
| total_installation_qty | 全艦裝置數 | INT | 📝 | 全艦總數 |
| maintenance_level | 裝備維修等級代碼 | VARCHAR(10) | 📝 | 維修等級 |
| equipment_serial | 裝備序號 | VARCHAR(50) | UNIQUE | 裝備識別編號 |
| date_created | 建立時間 | TIMESTAMP | 🔄 | |
| date_updated | 更新時間 | TIMESTAMP | 🔄 | |

**外鍵關聯**:
- `item_uuid` → `Item.item_uuid` (ON DELETE CASCADE)

---

### 4. Item_Material_Ext（料件擴展表）

**用途**: 料件類型專用欄位（SEMI/RM 類型）
**來源**: 原 Item 表屬性欄位

| 英文欄位名 | 中文名稱 | 資料類型 | 標記 | 說明 |
|-----------|---------|---------|------|------|
| item_uuid | 品項UUID | UUID | 🔑🔗 | 外鍵連結至 Item |
| item_id_last5 | 品項識別碼(後五碼) | VARCHAR(5) | 📝 | 快速識別用 |
| item_name_zh_short | 中文品名（9字內） | VARCHAR(20) | 📝 | 簡短中文名 |
| nsn | NSN/國家料號 | VARCHAR(20) | UNIQUE | NATO Stock Number |
| category_code | 統一組類別 | VARCHAR(10) | 📝 | 品項分類代碼 |
| item_code | 品名代號 | VARCHAR(10) | 📝 | INC品名代號 |
| fiig | FIIG | VARCHAR(10) | 📝 | 聯邦品項識別指南 |
| weapon_system_code | 武器系統代號 | VARCHAR(20) | 📝 | 所屬武器系統 |
| accounting_unit_code | 會計編號 | VARCHAR(20) | 📝 | 會計科目代號 |
| issue_unit | 撥發單位 | VARCHAR(10) | 📝 | EA/SET/LOT等 |
| unit_price | 美金單價 | DECIMAL(10,2) | 📝 | 單位價格（美金） |
| unit_pack_quantity | 單位包裝量 | INT | 📝 | 包裝數量 |
| weight_kg | 重量(KG) | DECIMAL(10,3) | 📝 | 單位重量（公斤） |
| has_stock | 有無料號 | BOOLEAN | 📝 | 是否有庫存料號 |
| storage_life_months | 存儲壽限代號 | VARCHAR(10) | 📝 | 儲存期限代碼 |
| file_type_code | 檔別代號 | VARCHAR(10) | 📝 | 檔案類型代號 |
| file_type_category | 檔別區分 | VARCHAR(10) | 📝 | 檔案分類 |
| secrecy_code | 機密性代號 | VARCHAR(10) | 📝 | 機密等級（U/C/S等） |
| expendability_code | 消耗性代號 | VARCHAR(10) | 📝 | 消耗品分類（M/N等） |
| spec_indicator | 規格指示 | VARCHAR(10) | 📝 | 規格指標 |
| navy_source | 海軍軍品來源 | VARCHAR(50) | 📝 | 來源說明 |
| storage_type_code | 儲存型式 | VARCHAR(20) | 📝 | 儲存方式 |
| storage_life_action_code | 處理代號 (壽限處理) | VARCHAR(10) | 📝 | 壽限管理代號 |
| manufacturability_code | 製造能量 | VARCHAR(10) | 📝 | 製造能力 |
| repairability_code | 修理能量 | VARCHAR(10) | 📝 | 修理能力 |
| source_code | 來源代號 | VARCHAR(10) | 📝 | 來源分類 |
| project_code | 專案代號 | VARCHAR(20) | 📝 | 所屬專案 |
| date_created | 建立時間 | TIMESTAMP | 🔄 | |
| date_updated | 更新時間 | TIMESTAMP | 🔄 | |
| inc_code | INC代碼 | VARCHAR(20) | 📝 | INC分類碼 |
| fiig_code | FIIG代碼 | VARCHAR(20) | 📝 | FIIG碼 |

**備註**:
- `inc_code`: 用於儲存 INC 分類代碼，與 public.inc 表關聯
- `fiig_code`: 用於儲存 FIIG 代碼，與 public.fiig 表關聯

---

### 5. BOM（BOM主表）

**用途**: BOM 版本控制
**V3.0 新增**: 支援版本管理和歷史追溯

| 英文欄位名 | 中文名稱 | 資料類型 | 標記 | 說明 |
|-----------|---------|---------|------|------|
| bom_uuid | BOM UUID | UUID | 🔑🔄 | 自動生成 UUID |
| item_uuid | 成品料號 | UUID | 🔗⭐ | 外鍵連結至 Item |
| bom_code | BOM編號 | VARCHAR(50) | 📝 | BOM 識別碼 |
| revision | 版次 | VARCHAR(20) | 📝 | 版本號 (1.0, 1.1...) |
| effective_from | 生效日 | DATE | 📝 | 開始生效日期 |
| effective_to | 失效日 | DATE | 📝 | 結束生效日期 |
| status | 狀態 | VARCHAR(20) | 📝 | Released/Draft |
| remark | 備註 | TEXT | 📝 | 備註說明 |
| date_created | 建立時間 | TIMESTAMP | 🔄 | |
| date_updated | 更新時間 | TIMESTAMP | 🔄 | |

---

### 6. BOM_LINE（BOM明細行）⭐

**用途**: BOM 元件清單（Item 自我關聯）
**V3.0 核心**: 實現多層級 BOM 結構

| 英文欄位名 | 中文名稱 | 資料類型 | 標記 | 說明 |
|-----------|---------|---------|------|------|
| line_uuid | 行UUID | UUID | 🔑🔄 | 自動生成 UUID |
| bom_uuid | BOM UUID | UUID | 🔗⭐ | 外鍵連結至 BOM |
| line_no | 行號 | INT | ⭐ | 排序用 |
| component_item_uuid | 元件料號 | UUID | 🔗⭐ | 外鍵連結至 Item（元件） |
| qty_per | 單位用量 | DECIMAL(10,4) | ⭐ | 每單位成品需要數量 |
| scrap_type | 損耗型態 | VARCHAR(20) | 📝 | 損耗計算方式 |
| scrap_rate | 損耗率 | DECIMAL(5,4) | 📝 | 損耗百分比 |
| uom | 用量單位 | VARCHAR(10) | 📝 | 預設跟元件UOM一致 |
| position | 裝配位置 | VARCHAR(100) | 📝 | 裝配位置/站別 |
| remark | 備註 | TEXT | 📝 | 備註說明 |
| date_created | 建立時間 | TIMESTAMP | 🔄 | |
| date_updated | 更新時間 | TIMESTAMP | 🔄 | |

**外鍵關聯**:
- `bom_uuid` → `BOM.bom_uuid` (ON DELETE CASCADE)
- `component_item_uuid` → `Item.item_uuid` (ON DELETE CASCADE)

---

### 7. MRC（品項規格表）

**用途**: 記錄品項的規格資料
**V3.0 變更**: 取代 ItemSpecification，使用 UUID 主鍵

| 英文欄位名 | 中文名稱 | 資料類型 | 標記 | 說明 |
|-----------|---------|---------|------|------|
| mrc_uuid | MRC UUID | UUID | 🔑🔄 | 自動生成 UUID |
| item_uuid | 品項UUID | UUID | 🔗⭐ | 外鍵連結至 Item |
| spec_no | 規格順序 | INT | 📝 | 順序編號 |
| spec_abbr | 規格資料縮寫 | VARCHAR(20) | 📝 | 規格簡稱 |
| spec_en | 規格資料英文 | VARCHAR(200) | 📝 | 規格項目英文 |
| spec_zh | 規格資料翻譯 | VARCHAR(200) | 📝 | 規格項目中文 |
| answer_en | 英答 | VARCHAR(200) | 📝 | 規格值英文 |
| answer_zh | 中答 | VARCHAR(200) | 📝 | 規格值中文 |
| date_created | 建立時間 | TIMESTAMP | 🔄 | |
| date_updated | 更新時間 | TIMESTAMP | 🔄 | |

---

### 8. item_number_xref（零件號碼關聯檔）

**用途**: 品項-零件號-廠商的多對多關聯
**來源**: 20M_料號主要件號檔
**表名**: 實際資料庫表名為 `item_number_xref`（小寫，底線分隔）

| 英文欄位名 | 中文名稱 | 資料類型 | 標記 | 說明 |
|-----------|---------|---------|------|------|
| part_number_id | 零件號碼ID | SERIAL | 🔑🔄 | 自動編號 |
| part_number_reference | 配件號碼 | VARCHAR(50) | ⭐ | P/N |
| item_uuid | 品項UUID | UUID | 🔗⭐ | 外鍵連結至 Item |
| supplier_id | 廠商ID | INT | 🔗 | 外鍵連結至 Supplier |
| pn_acquisition_level | 參考號獲得程度 | VARCHAR(10) | 📝 | 取得難易度 |
| pn_acquisition_source | 參考號獲得來源 | VARCHAR(50) | 📝 | 取得管道 |
| is_primary | 是否為主要零件號 | BOOLEAN | 📝 | 主/替代零件號 |
| date_created | 建立時間 | TIMESTAMP | 🔄 | |
| date_updated | 更新時間 | TIMESTAMP | 🔄 | |

---

### 9. TechnicalDocument（技術文件檔）

**用途**: 管理技術文件與圖面資料

| 英文欄位名 | 中文名稱 | 資料類型 | 標記 | 說明 |
|-----------|---------|---------|------|------|
| document_id | 文件ID | SERIAL | 🔑🔄 | 自動編號 |
| document_reference | 圖名/書名 | VARCHAR(200) | ⭐ | 文件名稱 |
| document_version | 版次 | VARCHAR(20) | 📝 | 版本號 |
| shipyard_drawing_no | 船廠圖號 | VARCHAR(50) | 📝 | 船廠圖面編號 |
| design_drawing_no | 設計圖號 | VARCHAR(50) | 📝 | 設計圖面編號 |
| document_type | 資料類型 | VARCHAR(20) | 📝 | 文件類型 |
| document_category | 資料類別 | VARCHAR(20) | 📝 | 文件分類 |
| language | 語言 | VARCHAR(10) | 📝 | 中文/英文 |
| security_level | 機密等級 | VARCHAR(10) | 📝 | 機密分級 |
| eswbs_code | ESWBS（五碼） | VARCHAR(20) | 📝 | 裝備分類碼 |
| accounting_code | 會計編號 | VARCHAR(20) | 📝 | 會計科目 |
| date_created | 建立時間 | TIMESTAMP | 🔄 | |
| date_updated | 更新時間 | TIMESTAMP | 🔄 | |

---

### 10. Item_Document_xref（品項文件關聯檔）

**用途**: 品項-技術文件多對多關聯
**V3.0 變更**: 從 Equipment_Document_xref 改為 Item_Document_xref

| 英文欄位名 | 中文名稱 | 資料類型 | 標記 | 說明 |
|-----------|---------|---------|------|------|
| item_uuid | 品項UUID | UUID | 🔑🔗⭐ | 外鍵連結至 Item |
| document_id | 文件ID | INT | 🔑🔗⭐ | 外鍵連結至 TechnicalDocument |
| date_created | 建立時間 | TIMESTAMP | 🔄 | |
| date_updated | 更新時間 | TIMESTAMP | 🔄 | |

**複合主鍵**: (item_uuid, document_id)

---

### 11. SupplierCodeApplication（廠商代號申請表）

**用途**: 廠商代號申請表單
**V3.1 保留**: 與 Application 表用途不同，繼續保留

| 英文欄位名 | 中文名稱 | 資料類型 | 標記 | 說明 |
|-----------|---------|---------|------|------|
| application_uuid | 申編單UUID | UUID | 🔑🔄 | 自動生成 UUID |
| form_no | 流水號 | VARCHAR(50) | UNIQUE | 申請單流水號 |
| applicant | 申請人 | VARCHAR(50) | 📝 | 申請人姓名 |
| supplier_id | 廠商ID | INT | 🔗 | 外鍵連結至 Supplier (可選) |
| supplier_name | 廠商名稱 | VARCHAR(200) | 📝 | 廠商名稱 |
| address | 地址 | VARCHAR(200) | 📝 | 廠商地址 |
| phone | 電話 | VARCHAR(50) | 📝 | 聯絡電話 |
| business_items | 營業項目 | VARCHAR(200) | 📝 | 營業項目說明 |
| supplier_code | 廠家代號 | VARCHAR(20) | 📝 | 申請或現有代號 |
| equipment_name | 裝備名稱 | VARCHAR(200) | 📝 | 相關裝備名稱 |
| custom_fields | 自定義欄位 | JSONB | 📝 | 動態擴充欄位 |
| status | 狀態 | VARCHAR(20) | 📝 | Draft/Submitted等 |
| date_created | 建立時間 | TIMESTAMP | 🔄 | |
| date_updated | 更新時間 | TIMESTAMP | 🔄 | |

---

### 12. CIDApplication（CID申請單）

**用途**: CID 申請表單
**V3.1 保留**: 與 Application 表用途不同，繼續保留

| 英文欄位名 | 中文名稱 | 資料類型 | 標記 | 說明 |
|-----------|---------|---------|------|------|
| application_uuid | 申編單UUID | UUID | 🔑🔄 | 自動生成 UUID |
| form_no | 流水號 | VARCHAR(50) | UNIQUE | 申請單流水號 |
| applicant | 申請人 | VARCHAR(50) | 📝 | 申請人姓名 |
| item_uuid | 品項UUID | UUID | 🔗 | 外鍵連結至 Item (可選) |
| suggested_prefix | 建議前兩碼 | VARCHAR(2) | 📝 | 建議CID前綴 |
| approved_cid | 核定CID | VARCHAR(50) | 📝 | 核定後的CID |
| equipment_name_zh | 裝備中文名稱 | VARCHAR(100) | 📝 | 中文名稱 |
| equipment_name_en | 裝備英文名稱 | VARCHAR(200) | 📝 | 英文名稱 |
| supplier_code | 廠家代號 | VARCHAR(20) | 📝 | 相關廠商代號 |
| part_number | 配件號碼 | VARCHAR(50) | 📝 | P/N |
| status | 狀態 | VARCHAR(20) | 📝 | Draft/Submitted等 |
| date_created | 建立時間 | TIMESTAMP | 🔄 | |
| date_updated | 更新時間 | TIMESTAMP | 🔄 | |

---

### 13. item_emu3000_maintenance_ext（EMU3000維修物料擴展表）

**用途**: 儲存 EMU3000 維修物料清單專屬欄位
**來源**: EMU3000 PDF/Excel 匯入

| 英文欄位名 | 中文名稱 | 資料類型 | 標記 | 說明 |
|-----------|---------|---------|------|------|
| ext_uuid | 擴展表UUID | UUID | 🔑🔄 | 自動生成 UUID |
| item_uuid | 品項UUID | UUID | 🔗 | 外鍵連結至 Item |
| part | 項次/行號 | VARCHAR(255) | 📝 | 原始資料 Item No/Part |
| quantity | 數量 | NUMERIC(10, 4) | 📝 | 數量 |
| unit | 單位 | VARCHAR(50) | 📝 | 單位 (Pce, SET等) |
| name | 品名 | VARCHAR(255) | 📝 | 原始品名 |
| wec | WEC編碼 | VARCHAR(100) | 📝 | WEC Code |
| notes | 備註 | TEXT | 📝 | 備註說明 |
| kit_no | 套件編號 | VARCHAR(100) | 📝 | Kit Number |
| date_created | 建立時間 | TIMESTAMP | 🔄 | |
| date_updated | 更新時間 | TIMESTAMP | 🔄 | |

**外鍵關聯**:
- `item_uuid` → `Item.item_uuid` (ON DELETE CASCADE)

---

## web_app Schema - Web 應用表（6個）

> **Schema**: `web_app`  
> **說明**: Web 應用核心表，包含使用者、申編單、附件、日誌等  
> **V3.1 新增**: 2025-11-25 整合至資料庫

### 14. User（使用者管理）⭐

**用途**: 系統使用者帳號管理
**V3.1 新增**: Web 應用核心表

| 英文欄位名 | 中文名稱 | 資料類型 | 標記 | 說明 |
|-----------|---------|---------|------|------|
| id | 使用者UUID | UUID | 🔑🔄 | 自動生成 UUID |
| username | 使用者帳號 | VARCHAR(80) | ⭐ UNIQUE | 登入帳號 |
| email | 電子郵件 | VARCHAR(120) | ⭐ UNIQUE | 電子郵件地址 |
| password_hash | 密碼雜湊值 | VARCHAR(256) | ⭐ | 加密後的密碼 |
| english_code | 英文代碼 | VARCHAR(10) | 📝 | 英文代號 |
| full_name | 全名 | VARCHAR(100) | 📝 | 完整姓名 |
| department | 部門 | VARCHAR(100) | 📝 | 所屬部門 |
| position | 職位 | VARCHAR(100) | 📝 | 職稱 |
| phone | 電話 | VARCHAR(20) | 📝 | 聯絡電話 |
| role | 角色權限 | VARCHAR(50) | 📝 | admin/user/viewer等 |
| is_active | 帳號啟用狀態 | BOOLEAN | 📝 | 帳號是否啟用 |
| is_verified | 電子郵件驗證狀態 | BOOLEAN | 📝 | Email是否已驗證 |
| email_verified_at | 電子郵件驗證時間 | TIMESTAMP | 📝 | 驗證時間戳 |
| last_login_at | 最後登入時間 | TIMESTAMP | 📝 | 最後登入時間戳 |
| failed_login_attempts | 登入失敗次數 | INT | 📝 | 連續失敗次數 |
| locked_until | 帳號鎖定至 | TIMESTAMP | 📝 | 鎖定解除時間 |
| date_created | 建立時間 | TIMESTAMP | 🔄 | 記錄建立時間 |
| date_updated | 更新時間 | TIMESTAMP | 🔄 | 記錄更新時間 |

**索引**:
- `idx_user_email` - Email索引
- `idx_user_username` - 使用者名稱索引
- `idx_user_role` - 角色索引
- `idx_user_is_active` - 啟用狀態索引

---

### 15. Application（申編單主表）⭐

**用途**: 申編單核心資料管理
**V3.2 精簡化**: 僅保留核心欄位，詳細資料儲存於關聯表

| 英文欄位名 | 中文名稱 | 資料類型 | 標記 | 說明 |
|-----------|---------|---------|------|------|
| id | 申編單UUID | UUID | 🔑🔄 | 自動生成 UUID |
| user_id | 申請人 | UUID | 🔗 | 外鍵至User表 |
| item_uuid | 關聯品項UUID | UUID | 🔗 | 外鍵至Item表 ⭐ |
| date_created | 建立時間 | TIMESTAMP | 🔄 | 記錄建立時間 |
| date_updated | 更新時間 | TIMESTAMP | 🔄 | 記錄更新時間 |
| deleted_at | 軟刪除時間戳 | TIMESTAMP | 📝 | 軟刪除標記 |
| form_serial_number | 表單序號 | VARCHAR(50) | 📝 | 申編單編號 |
| system_code | 系統代碼 | VARCHAR(100) | 📝 | 系統分類 |
| mrc_data | MRC資料 | JSON | 📝 | 規格資料（JSON格式） |
| applicant_unit | 申請單位 | VARCHAR(100) | 📝 | 申請部門 |
| contact_info | 聯絡資訊 | VARCHAR(100) | 📝 | 聯絡方式 |
| apply_date | 申請日期 | DATE | 📝 | 申請日 |
| official_nsn_stamp | 正式NSN印章 | VARCHAR(10) | 📝 | NSN章戳 |
| official_nsn_final | 最終正式NSN | VARCHAR(20) | 📝 | 正式NSN號碼 |
| nsn_filled_at | NSN填寫時間 | TIMESTAMP | 📝 | NSN填寫時間戳 |
| nsn_filled_by | NSN填寫者 | UUID | 🔗 | 外鍵至User表 |
| closed_at | 結案時間 | TIMESTAMP | 📝 | 結案時間戳 |
| closed_by | 結案者 | UUID | 🔗 | 外鍵至User表 |
| status | 申編單狀態 | VARCHAR(50) | 📝 | 狀態（pending/approved等） |
| sub_status | 申編單子狀態 | VARCHAR(50) | 📝 | 詳細狀態 |

**外鍵關聯**:
- `user_id` → `User.id`
- `item_uuid` → `Item.item_uuid` (ON DELETE SET NULL) ⭐
- `nsn_filled_by` → `User.id`
- `closed_by` → `User.id`

**索引**:
- `idx_app_user_id` - 使用者索引
- `idx_app_item_uuid` - 品項索引 ⭐
- `idx_app_status` - 狀態索引
- `idx_app_date_created` - 建立時間索引
- `idx_app_deleted_at` - 軟刪除索引（部分索引）

**備註**:
- V3.2 精簡化：移除大部分料件屬性欄位
- 料件詳細資訊透過 `item_uuid` 關聯至擴展表：
  - `item_material_ext`: 料件屬性（accounting_unit_code, unit_price等）
  - `item_equipment_ext`: 裝備屬性（usage_location等）
  - `item_number_xref`: 零件號碼（pn_acquisition_level等）
  - `technicaldocument`: 技術文件（document_reference等）

#### Application 表欄位對應表（舊 → 新）

下表顯示舊版 `web_app.applications` 表（51 欄位）與新版 `web_app.application` 表（20 欄位）的欄位對應關係：

| 舊版欄位 (applications) | 新版欄位 (application) | 資料遷移說明 |
|------------------------|----------------------|-------------|
| **核心欄位（保留）** | | |
| `id` | `id` | ✅ 直接對應 |
| `user_id` | `user_id` | ✅ 直接對應 |
| `form_serial_number` | `form_serial_number` | ✅ 直接對應 |
| `status` | `status` | ✅ 直接對應 |
| `sub_status` | `sub_status` | ✅ 直接對應 |
| `created_at` | `date_created` | ✅ 欄位改名 |
| `updated_at` | `date_updated` | ✅ 欄位改名 |
| `deleted_at` | `deleted_at` | ✅ 直接對應 |
| `system_code` | `system_code` | ✅ 直接對應 |
| `mrc_data` | `mrc_data` | ✅ 直接對應（JSON） |
| `applicant_unit` | `applicant_unit` | ✅ 直接對應 |
| `contact_info` | `contact_info` | ✅ 直接對應 |
| `apply_date` | `apply_date` | ✅ 直接對應 |
| `official_nsn_stamp` | `official_nsn_stamp` | ✅ 直接對應 |
| `official_nsn_final` | `official_nsn_final` | ✅ 直接對應 |
| `nsn_filled_at` | `nsn_filled_at` | ✅ 直接對應 |
| `nsn_filled_by` | `nsn_filled_by` | ✅ 直接對應 |
| `closed_at` | `closed_at` | ✅ 直接對應 |
| `closed_by` | `closed_by` | ✅ 直接對應 |
| **新增欄位** | | |
| - | `item_uuid` | 🆕 關聯至 Item 表（核心整合點） |
| **移除欄位（料件屬性）** | | |
| `part_number` | ❌ 移除 | → `Item.item_code` |
| `english_name` | ❌ 移除 | → `Item.item_name_en` |
| `chinese_name` | ❌ 移除 | → `Item.item_name_zh` |
| `inc_code` | ❌ 移除 | → `Item_Material_Ext.inc_code` |
| `fiig_code` | ❌ 移除 | → `Item_Material_Ext.fiig_code` |
| `accounting_unit_code` | ❌ 移除 | → `Item_Material_Ext.accounting_unit_code` |
| `issue_unit` | ❌ 移除 | → `Item_Material_Ext.issue_unit` |
| `unit_price` | ❌ 移除 | → `Item_Material_Ext.unit_price` |
| `spec_indicator` | ❌ 移除 | → `Item_Material_Ext.spec_indicator` |
| `unit_pack_quantity` | ❌ 移除 | → `Item_Material_Ext.unit_pack_quantity` |
| `storage_life_months` | ❌ 移除 | → `Item_Material_Ext.storage_life_months` |
| `storage_life_action_code` | ❌ 移除 | → `Item_Material_Ext.storage_life_action_code` |
| `storage_type_code` | ❌ 移除 | → `Item_Material_Ext.storage_type_code` |
| `secrecy_code` | ❌ 移除 | → `Item_Material_Ext.secrecy_code` |
| `expendability_code` | ❌ 移除 | → `Item_Material_Ext.expendability_code` |
| `repairability_code` | ❌ 移除 | → `Item_Material_Ext.repairability_code` |
| `manufacturability_code` | ❌ 移除 | → `Item_Material_Ext.manufacturability_code` |
| `source_code` | ❌ 移除 | → `Item_Material_Ext.source_code` |
| `category_code` | ❌ 移除 | → `Item_Material_Ext.category_code` |
| **移除欄位（裝備屬性）** | | |
| `ship_type` | ❌ 移除 | → `Item_Equipment_Ext.ship_type` |
| `model_type` | ❌ 移除 | → `Item_Equipment_Ext.equipment_type` |
| `equipment_name` | ❌ 移除 | → `Item.item_name_zh/en` |
| `usage_location` | ❌ 移除 | → `Item_Equipment_Ext.usage_location` |
| `quantity_per_unit` | ❌ 移除 | → `Item_Material_Ext.unit_pack_quantity` |
| **移除欄位（零件號碼）** | | |
| `pn_acquisition_level` | ❌ 移除 | → `item_number_xref.pn_acquisition_level` |
| `pn_acquisition_source` | ❌ 移除 | → `item_number_xref.pn_acquisition_source` |
| `part_number_reference` | ❌ 移除 | → `item_number_xref.part_number_reference` |
| **移除欄位（廠商資訊）** | | |
| `manufacturer` | ❌ 移除 | → `Supplier.supplier_name_en` |
| `manufacturer_name` | ❌ 移除 | → `Supplier.supplier_name_en` |
| `agent_name` | ❌ 移除 | → `Supplier.agent_name` |
| **移除欄位（其他）** | | |
| `cid_no` | ❌ 移除 | → `Item.item_code` (CID) |
| `document_reference` | ❌ 移除 | → `TechnicalDocument.document_reference` |

**欄位統計**:
- 舊版總欄位數: **51 個**
- 新版總欄位數: **20 個**
- 保留欄位: **17 個**（含 2 個改名欄位：created_at→date_created, updated_at→date_updated）
- 新增欄位: **1 個** (`item_uuid`)
- 移除欄位: **34 個**（包含 32 個移至關聯表 + 2 個重複時間欄位）

**資料存取方式**:
```sql
-- 舊版查詢（所有資料在一個表）
SELECT part_number, english_name, manufacturer 
FROM web_app.applications 
WHERE id = 'xxx';

-- 新版查詢（透過關聯）
SELECT 
    a.form_serial_number,
    i.item_code AS part_number,
    i.item_name_en AS english_name,
    s.supplier_name_en AS manufacturer
FROM web_app.application a
LEFT JOIN web_app.item i ON a.item_uuid = i.item_uuid
LEFT JOIN web_app.item_material_ext ime ON i.item_uuid = ime.item_uuid
LEFT JOIN web_app.item_number_xref inx ON i.item_uuid = inx.item_uuid
LEFT JOIN web_app.supplier s ON inx.supplier_id = s.supplier_id
WHERE a.id = 'xxx';
```

---

### 16. ApplicationAttachment（附件管理）

**用途**: 申編單附件（BYTEA儲存）
**V3.1 新增**: 支援二進制檔案儲存

| 英文欄位名 | 中文名稱 | 資料類型 | 標記 | 說明 |
|-----------|---------|---------|------|------|
| id | 附件UUID | UUID | 🔑🔄 | 自動生成 UUID |
| application_id | 申編單ID | UUID | 🔗⭐ | 外鍵至Application表 |
| user_id | 上傳者 | UUID | 🔗 | 外鍵至User表 |
| file_data | 檔案二進制資料 | BYTEA | 📝 | 檔案內容（二進制） |
| filename | 儲存檔名 | VARCHAR(255) | 📝 | 系統儲存檔名 |
| original_filename | 原始檔名 | VARCHAR(255) | 📝 | 使用者上傳檔名 |
| mimetype | MIME類型 | VARCHAR(100) | 📝 | 檔案類型 |
| file_type | 檔案類型 | VARCHAR(20) | 📝 | PDF/PNG/JPG等 |
| page_selection | 頁面選擇 | VARCHAR(200) | 📝 | PDF多頁時選擇 |
| sort_order | 排序順序 | INT | 📝 | 附件排序 |
| date_created | 建立時間 | TIMESTAMP | 🔄 | 上傳時間 |
| date_updated | 更新時間 | TIMESTAMP | 🔄 | 更新時間 |

**外鍵關聯**:
- `application_id` → `Application.id` (ON DELETE CASCADE)
- `user_id` → `User.id`

---

### 17. UserSession（工作階段管理）

**用途**: 使用者登入Session管理
**V3.1 新增**: 支援 remember_me 功能

| 英文欄位名 | 中文名稱 | 資料類型 | 標記 | 說明 |
|-----------|---------|---------|------|------|
| session_id | Session ID | VARCHAR(255) | 🔑 | Session識別碼 |
| user_id | 使用者ID | UUID | 🔗⭐ | 外鍵至User表 |
| ip_address | IP位址 | VARCHAR(45) | 📝 | 登入IP |
| user_agent | 使用者代理字串 | TEXT | 📝 | 瀏覽器UA |
| is_active | 是否啟用 | BOOLEAN | 📝 | Session是否有效 |
| remember_me | 記住我功能 | BOOLEAN | 📝 | 是否記住登入 |
| expires_at | Session過期時間 | TIMESTAMP | 📝 | 過期時間戳 |
| date_created | 建立時間 | TIMESTAMP | 🔄 | Session建立時間 |
| last_activity_at | 最後活動時間 | TIMESTAMP | 📝 | 最後活動時間戳 |

**外鍵關聯**:
- `user_id` → `User.id` (ON DELETE CASCADE)

---

### 18. ApplicationLog（應用程式日誌）

**用途**: 應用程式運行日誌
**V3.1 新增**: 支援完整的日誌記錄與除錯

| 英文欄位名 | 中文名稱 | 資料類型 | 標記 | 說明 |
|-----------|---------|---------|------|------|
| log_id | 日誌UUID | UUID | 🔑🔄 | 自動生成 UUID |
| timestamp | 日誌時間戳 | TIMESTAMPTZ | 🔄 | 時間戳（含時區） |
| level | 日誌等級 | VARCHAR(10) | 📝 | DEBUG/INFO/WARNING/ERROR |
| logger | 日誌記錄器名稱 | VARCHAR(100) | 📝 | Logger名稱 |
| message | 日誌訊息 | TEXT | 📝 | 日誌內容 |
| request_id | 請求ID | VARCHAR(36) | 📝 | 請求追蹤ID |
| method | HTTP方法 | VARCHAR(10) | 📝 | GET/POST/PUT/DELETE等 |
| path | 請求路徑 | VARCHAR(500) | 📝 | API路徑 |
| status_code | HTTP狀態碼 | INT | 📝 | 200/404/500等 |
| elapsed_time_ms | 請求處理時間（毫秒） | NUMERIC(10,2) | 📝 | 執行時間 |
| user_id | 使用者ID | UUID | 📝 | 相關使用者 |
| remote_addr | 遠端IP位址 | INET | 📝 | 客戶端IP |
| user_agent | 使用者代理字串 | TEXT | 📝 | 瀏覽器UA |
| module | 模組名稱 | VARCHAR(100) | 📝 | Python模組 |
| function | 函數名稱 | VARCHAR(100) | 📝 | 函數名稱 |
| line | 行號 | INT | 📝 | 程式碼行號 |
| exception_type | 異常類型 | VARCHAR(100) | 📝 | Exception類別 |
| exception_message | 異常訊息 | TEXT | 📝 | 錯誤訊息 |
| exception_traceback | 異常堆疊追蹤 | JSONB | 📝 | 完整Traceback（JSON） |
| extra_fields | 額外欄位 | JSONB | 📝 | 自定義欄位（JSON） |
| created_date | 日誌日期 | DATE | 🔄 | 日期（用於分區） |

**索引**:
- `idx_app_log_timestamp` - 時間戳索引
- `idx_app_log_level` - 日誌等級索引
- `idx_app_log_user_id` - 使用者索引
- `idx_app_log_created_date_timestamp` - 複合索引（日期+時間）

---

### 19. AuditLog（稽核日誌）

**用途**: 操作審計追蹤
**V3.1 新增**: 完整的 Audit Trail 功能

| 英文欄位名 | 中文名稱 | 資料類型 | 標記 | 說明 |
|-----------|---------|---------|------|------|
| log_id | 日誌UUID | UUID | 🔑🔄 | 自動生成 UUID |
| user_id | 使用者ID | UUID | 🔗 | 外鍵至User表 |
| action | 操作動作 | VARCHAR(100) | 📝 | CREATE/UPDATE/DELETE等 |
| resource_type | 資源類型 | VARCHAR(50) | 📝 | Application/Item等 |
| resource_id | 資源ID | VARCHAR(100) | 📝 | 被操作資源的ID |
| old_values | 修改前舊值 | JSON | 📝 | 修改前的資料（JSON） |
| new_values | 修改後新值 | JSON | 📝 | 修改後的資料（JSON） |
| ip_address | IP位址 | VARCHAR(45) | 📝 | 操作者IP |
| user_agent | 使用者代理字串 | TEXT | 📝 | 瀏覽器UA |
| success | 操作是否成功 | BOOLEAN | 📝 | 操作結果 |
| error_message | 錯誤訊息 | TEXT | 📝 | 失敗原因 |
| date_created | 建立時間 | TIMESTAMP | 🔄 | 操作時間 |

**外鍵關聯**:
- `user_id` → `User.id`

**索引**:
- `idx_audit_user_id` - 使用者索引
- `idx_audit_action` - 操作動作索引
- `idx_audit_resource` - 資源類型+ID複合索引
- `idx_audit_date_created` - 建立時間索引

---

## public Schema - NSN 搜尋系統（15個表 + 5個視圖）

> **Schema**: `public`  
> **說明**: NSN 申編搜尋系統，提供 NATO 標準物品名稱查詢與申編流程  
> **建立日期**: 2025-12-08/09  
> **資料量**: 超過 50 萬筆記錄

### 系統架構

NSN 搜尋系統由以下子系統組成：

1. **FSG/FSC 分類系統**：聯邦補給群組與分類
2. **NATO H6 系統**：NATO 標準物品名稱（6 位編碼）
3. **INC 系統**：物品名稱代碼（5 位編碼）
4. **FIIG 系統**：聯邦物品識別指南（4 位編碼）
5. **MRC 系統**：物料需求代碼與申編規格
6. **回應系統**：申編表單回應選項

### 資料表詳細說明

#### FSG/FSC 分類系統

**20. fsg（FSG 聯邦補給群組）**
- **主鍵**: fsg_code (VARCHAR(2))
- **記錄數**: 80 筆
- **說明**: 聯邦補給群組分類（如 58=通訊設備）

**21. fsc（FSC 聯邦補給分類）**
- **主鍵**: fsc_code (VARCHAR(4))
- **記錄數**: 約 650 筆
- **說明**: 聯邦補給分類代碼（FSG 下的細分類）

**22. inc_fsc_xref（INC-FSC 對應表）**
- **複合主鍵**: (inc_code, fsc_code)
- **記錄數**: 約 78,000 筆
- **說明**: INC 與 FSC 的多對多對應關係

#### NATO H6 系統

**23. nato_h6_item_name（NATO H6 物品名稱）**
- **主鍵**: h6_code (VARCHAR(6))
- **記錄數**: 60,698 筆
- **說明**: NATO 標準物品名稱主檔（如 002001=BOLT）
- **欄位**: h6_code, item_name_en, status_code, date_created, date_updated

**24. nato_h6_inc_xref（H6-INC 對應表）**
- **複合主鍵**: (h6_code, inc_code)
- **記錄數**: 約 60,000 筆
- **說明**: NATO H6 與 INC 的對應關係

#### INC 系統

**25. inc（INC 物品名稱代碼）**
- **主鍵**: inc_code (VARCHAR(5))
- **記錄數**: 79,865 筆（69,179 active + 10,686 inactive）
- **說明**: 物品名稱代碼主檔（如 00180=BOLT）
- **重要欄位**: inc_code, noun, approved_item_name, status_code

**26. colloquial_inc_xref（俗語-INC 對應表）**
- **複合主鍵**: (colloquial_name, inc_code)
- **記錄數**: 約 142,000 筆
- **說明**: 俗語/別名與 INC 的對應（支援模糊搜尋）

#### FIIG 系統

**27. fiig（FIIG 識別指南）**
- **主鍵**: fiig_code (VARCHAR(4))
- **記錄數**: 約 2,900 筆
- **說明**: 聯邦物品識別指南（如 A001）

**28. fiig_inc_xref（FIIG-INC 對應表）**
- **複合主鍵**: (fiig_code, inc_code)
- **記錄數**: 約 79,000 筆
- **說明**: FIIG 與 INC 的對應關係

#### MRC 系統

**29. mrc_key_group（MRC 群組）**
- **主鍵**: key_group (VARCHAR(3))
- **記錄數**: 約 50 筆
- **說明**: MRC 群組分類（如 001, 002...）

**30. mrc（MRC 需求代碼）**
- **主鍵**: mrc_code (VARCHAR(5))
- **記錄數**: 約 16,700 筆
- **說明**: 物料需求代碼主檔，定義申編表單必填欄位
- **重要欄位**: mrc_code, characteristic, key_group, seq_no

**31. fiig_inc_mrc_xref（FIIG-INC-MRC 對應表）⭐**
- **主鍵**: id (SERIAL)
- **記錄數**: 1,702,274 筆（170 萬筆，核心表）
- **說明**: FIIG-INC-MRC 三元對應，定義每個品項的申編需求
- **重要欄位**: fiig_code, inc_code, mrc_code, required_flag

**32. mrc_reply_table_xref（MRC-回應表對應）**
- **複合主鍵**: (mrc_code, reply_table_id)
- **記錄數**: 約 72,000 筆
- **說明**: MRC 與回應選項的對應關係

#### 回應系統

**33. reply_table（回應表主檔）**
- **主鍵**: id (SERIAL)
- **記錄數**: 160,591 筆
- **說明**: 申編表單回應選項主檔（下拉選單內容）
- **重要欄位**: reply, meaning, reply_code

#### 其他

**34. mode_code_edit（模式碼編輯指南）**
- **主鍵**: mode_code (VARCHAR(2))
- **記錄數**: 約 20 筆
- **說明**: 模式碼編輯規則（如 01=新增, 02=修改）

### 查詢視圖

**v_h6_inc_mapping**
- 將 NATO H6 轉換為 INC 的完整視圖
- 包含 H6 名稱、INC 代碼、核准名稱等

**v_inc_fiig_mapping**
- 將 INC 轉換為 FIIG 的完整視圖
- 支援 INC→FIIG 查詢

**v_fiig_mrc_requirements**
- 顯示每個 FIIG-INC 組合的 MRC 需求
- 用於生成申編表單必填欄位清單

**v_mrc_reply_options**
- 顯示每個 MRC 的可選回應選項
- 用於生成下拉選單內容

**v_application_flow**
- 端到端申編流程視圖
- 整合 H6→INC→FIIG→MRC→Reply 的完整流程

---

## 資料表關聯圖

### 雙 Schema 架構圖

```
sbir_equipment_db_v3 (PostgreSQL 16)
│
├─ web_app Schema（19 個表格）
│  ┌──────────────────────────────────────────────────┐
│  │         裝備管理系統（13 個表格）                │
│  │     (使用 date_created/date_updated)              │
│  │                                                   │
│  │  Supplier ─┬─ item_number_xref ─┬─ Item ⭐      │
│  │            │                      │   ├─ Item_Equipment_Ext
│  │            └─ SupplierCodeApp    │   ├─ Item_Material_Ext
│  │                                   │   ├─ item_emu3000_maintenance_ext
│  │                                   │   ├─ BOM → BOM_LINE (自我關聯)
│  │                                   │   ├─ MRC
│  │                                   │   ├─ Item_Document_xref → TechnicalDocument
│  │                                   │   └─ CIDApplication
│  └───────────────────────────────────────────────────┘
│                           │
│                           │ item_uuid (FK) ⭐
│                           ↓
│  ┌──────────────────────────────────────────────────┐
│  │         Web 應用系統（6 個表格）                 │
│  │     (統一使用 date_created/date_updated)          │
│  │                                                   │
│  │  User ─┬─ UserSession                            │
│  │        ├─ Application ───┬─ ApplicationAttachment│
│  │        │                  └─→ Item (FK) ⭐       │
│  │        ├─ AuditLog                               │
│  │        └─ ApplicationLog                         │
│  └───────────────────────────────────────────────────┘
│
└─ public Schema（15 個表格 + 5 個視圖）
   ┌──────────────────────────────────────────────────┐
   │      NSN 申編搜尋系統（50 萬筆資料）             │
   │     (使用 date_created/date_updated)              │
   │                                                   │
   │  FSG ─→ FSC ←─ inc_fsc_xref ─→ INC ⭐           │
   │                                    ↕              │
   │                         nato_h6_inc_xref          │
   │                                    ↕              │
   │                            NATO H6 Item Name      │
   │                                                   │
   │  INC ←─ colloquial_inc_xref (俗語查詢)           │
   │   ↕                                               │
   │  fiig_inc_xref ─→ FIIG                           │
   │   ↕                                               │
   │  fiig_inc_mrc_xref (170萬筆) ─→ MRC ←─ mrc_key_group
   │                                   ↕               │
   │                        mrc_reply_table_xref       │
   │                                   ↕               │
   │                              reply_table          │
   │                                                   │
   │  查詢視圖（5 個）:                                │
   │  - v_h6_inc_mapping                               │
   │  - v_inc_fiig_mapping                             │
   │  - v_fiig_mrc_requirements                        │
   │  - v_mrc_reply_options                            │
   │  - v_application_flow                             │
   └───────────────────────────────────────────────────┘
```

### Application 表的關聯重點

```
Application (申編單主表)
├─ user_id → User (申請人)
├─ item_uuid → Item (關聯品項) ⭐ 核心整合點
├─ nsn_filled_by → User (NSN填寫人)
├─ closed_by → User (結案人)
└─ 1:N → ApplicationAttachment (附件)
```

---

## 時間戳命名策略

### 統一命名規範

**所有表（34個）**: 統一使用 `date_created`, `date_updated`

**web_app Schema - 裝備管理表（13個）**:
- Supplier, Item, Item_Equipment_Ext, Item_Material_Ext, item_emu3000_maintenance_ext
- BOM, BOM_LINE, MRC, item_number_xref
- TechnicalDocument, Item_Document_xref
- SupplierCodeApplication, CIDApplication

**web_app Schema - Web 應用表（6個）**:
- User, Application, ApplicationAttachment, UserSession
- ApplicationLog（特殊：使用 `timestamp` + `created_date`）
- AuditLog

**public Schema - NSN 搜尋系統表（15個）**:
- fsg, fsc, inc_fsc_xref
- nato_h6_item_name, nato_h6_inc_xref
- inc, colloquial_inc_xref
- fiig, fiig_inc_xref
- mrc_key_group, mrc, fiig_inc_mrc_xref, mrc_reply_table_xref
- reply_table, mode_code_edit

**優點**:
- 整個資料庫保持命名一致性
- 簡化觸發器函數邏輯
- 降低開發人員記憶負擔

### 觸發器函數統一處理

```sql
CREATE OR REPLACE FUNCTION update_date_updated_column()
RETURNS TRIGGER AS $$
BEGIN
    -- 所有表統一使用 date_updated 欄位
    IF to_jsonb(NEW) ? 'date_updated' THEN
        NEW.date_updated = CURRENT_TIMESTAMP;
    END IF;
    RETURN NEW;
END;
$$ language 'plpgsql';
```

---

## EMU3000 資料匯入規則

### 來源資料
- **路徑**: `@Database/data/EMU3000/EMU 3000 維修物料清單/5年維護/01 清潔塊單元--C20、C21--OK/清潔塊單元C20、C21大修料件清單--60個月.xlsx` (範例)
- **格式**: Excel 檔案

### 識別邏輯

#### 1. 總成 (Assembly)
- **識別特徵**: `Part` 欄位為 `NULL` 或特定格式 (如 "II...")
- **主要欄位對應**:
    - `Item number` -> `Item.item_code` (識別碼)
    - `Name` -> `Item.item_name_zh` / `Item.item_name_en`
    - `Quantity` -> 不適用於 Item 表，可能用於上層 BOM
- **Item 類型**: `FG` (Finished Good) 或 `SEMI` (Semi-finished)

#### 2. 次總成/元件 (Sub-assembly / Component)
- **識別特徵**: `Part` 欄位為序號 (如 "1", "2"...)
- **主要欄位對應**:
    - `Item number` -> `Item.item_code`
    - `Name` -> `Item.item_name_zh` / `Item.item_name_en`
    - `Quantity` -> `BOM_LINE.qty_per` (單位用量)
    - `Unit` -> `BOM_LINE.uom`
- **Item 類型**: `SEMI` 或 `RM` (Raw Material)

### BOM 結構建立

1. **建立總成 Item**:
   - 將 Excel 表頭資訊或 `Part` 為 NULL 的行建立為總成 Item。
   - 建立一個對應的 `BOM` 記錄，關聯到此 Item。

2. **建立次總成 Item**:
   - 將 `Part` 為序號的每一行建立為次總成/元件 Item。

3. **建立 BOM 關聯**:
   - 在 `BOM_LINE` 表中建立記錄。
   - `bom_uuid`: 指向總成的 BOM。
   - `component_item_uuid`: 指向次總成/元件的 Item。
   - `qty_per`: 填入 Excel 中的 `Quantity`。
   - `line_no`: 填入 Excel 中的 `Part` (序號)。

---

## 📊 效能與擴展性

### ✅ 優點
- **雙 Schema 架構**：功能隔離與安全分離
- **支援多層級 BOM 結構**：Item 自我關聯設計
- **BOM 版本控制**：歷史追溯與版本管理
- **擴展表減少 NULL 欄位**：提升儲存效率
- **UUID 主鍵**：防止 ID 猜測，支援分散式系統
- **完整的使用者管理系統**：權限控制與審計追蹤
- **軟刪除支援**：deleted_at 欄位保留歷史記錄
- **NSN 搜尋系統**：50 萬筆資料，支援高效查詢
- **查詢視圖**：5 個視圖簡化複雜查詢
- **適合大量資料擴展**：支援百萬級資料量

### 📈 索引策略
- **外鍵欄位索引**：所有 FK 都有索引
- **常用查詢欄位索引**：status, code, name 等
- **部分索引**：deleted_at IS NULL 減少索引大小
- **複合索引**：支援多欄位查詢模式
- **NSN 系統索引**：fiig_inc_mrc_xref 有完整索引策略
- **視圖效能**：5 個視圖預先定義常用查詢

### 🔐 安全與隔離
- **Schema 隔離**：web_app 與 public 功能分離
- **權限控制**：可針對不同 schema 設定不同權限
- **資料安全**：密碼雜湊、軟刪除、審計日誌

---

**文件版本**: 3.2.0
**最後更新**: 2025-12-09
**維護單位**: SBIR 專案團隊
**資料表總數**: ✅ 34 個表（web_app: 19 + public: 15）+ 5 個視圖
**Schema 架構**: 雙 schema（web_app 裝備管理 + public 搜尋系統）
**總資料量**: 超過 220 萬筆記錄（web_app: 約 1,000 筆 + public: 約 220 萬筆）
**主要變更**: Schema 重構 + NSN 系統整合（15 表 + 5 視圖 + 50 萬筆資料）
