...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'mrc') THEN
        ...
        -- 重命名索引
        ALTER INDEX idx_mrc_item RENAME TO idx_item_mrc_answer_item;
        ALTER INDEX idx_mrc_abbr RENAME TO idx_item_mrc_answer_abbr;

        -- 重命名觸發器
        ALTER TRIGGER update_mrc_date_updated ON Item_MRC_Answer
            RENAME TO update_item_mrc_answer_date_updated;
...
-- 索引
CREATE INDEX idx_nsn_inc_status ON NSN_INC(status_code);
CREATE INDEX idx_nsn_inc_short_name ON NSN_INC(short_name);

-- 2.2 俗名 INC 對應表（選配）
CREATE TABLE NSN_Colloquial_INC_xref (
...
-- 索引
CREATE INDEX idx_nsn_fsc_fsg ON NSN_FSC(fsg_code);
...
COMMENT ON COLUMN NSN_INC_FSC_xref.inc_code IS 'INC代碼';
COMMENT ON COLUMN NSN_INC_FSC_xref.fsc_code IS 'FSC代碼';
...
-- 索引
CREATE INDEX idx_nsn_fiig_status ON NSN_FIIG(status_code);
...
COMMENT ON COLUMN NSN_FIIG_INC_xref.sort_order IS '顯示順序';

-- 索引
CREATE INDEX idx_nsn_fiig_inc_xref_inc ON NSN_FIIG_INC_xref(inc_code);
CREATE INDEX idx_nsn_fiig_inc_xref_fiig ON NSN_FIIG_INC_xref(fiig_code);
...
-- 索引
CREATE INDEX idx_nsn_mrc_key_group ON NSN_MRC(key_group_number);

-- ============================================
-- 步驟 6：修改 Item_Material_Ext 表（建立外鍵）
-- ============================================

-- 6.1 檢查 item_code 欄位是否可作為 INC 外鍵
-- 若 item_code 已有其他用途，應新增 inc_code 欄位
DO $$
BEGIN
    -- 檢查是否需要新增 inc_code 欄位（若 item_code 用途不同）
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_name = 'item_material_ext'
        AND column_name = 'inc_code'
    ) THEN
        -- 選項A：新增專用的 inc_code 欄位（推薦）
...
-- 6.2 建立 FIIG 外鍵約束（若欄位值已標準化）
-- 注意：僅在確認現有 fiig 值與 NSN_FIIG.fiig_code 格式一致時啟用
/*
ALTER TABLE Item_Material_Ext
    ADD CONSTRAINT fk_item_material_fiig
    FOREIGN KEY (fiig) REFERENCES NSN_FIIG(fiig_code);
*/

COMMENT ON COLUMN Item_Material_Ext.fiig IS 'FIIG代碼（可關聯到NSN_FIIG）';
...
BEGIN
    ...
    RAISE NOTICE '';
    RAISE NOTICE '下一步：';
    RAISE NOTICE '  1. 匯入完整 NSN 資料（INC/FSC/FIIG/MRC）';
    RAISE NOTICE '  2. 更新現有 Item_Material_Ext.inc_code 欄位';
    RAISE NOTICE '  3. 驗證資料關聯性';
    RAISE NOTICE '=========================================';
END $$;
